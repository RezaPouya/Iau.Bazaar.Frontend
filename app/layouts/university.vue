...
const pageTitle = computed(() => route.meta.title || 'پنل دانشگاه')
...
const navigation = [
  ...
  },
  {
    ...
    to: '/university/growth-centers'
  },
  {
    label: 'مدیریت کاربران مراکز رشد',
    icon: 'i-lucide-users',
    to: '/university/growth-center-users'
  },
  {
    label: 'مشاهده شرکت‌ها',
    icon: 'i-lucide-building',
    to: '/university/companies'
  },
  {
            ...
            <UDropdownMenu :items="userDropdownItems" :content="{ align: 'end' }">
              <UButton color="neutral" variant="ghost" class="w-full justify-start gap-3 px-2 py-2" :class="isSidebarCollapsed ? 'justify-center' : ''">
                ...
                <div v-if="!isSidebarCollapsed" class="flex-1 text-right overflow-hidden">
                  ...
                  <p class="text-xs text-dimmed truncate">دانشگاه</p>
                </div>
