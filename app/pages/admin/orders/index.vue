...
const orderService = useAdminOrderService()
...
const loadOrders = async () => {
  ...
  try {
    ...
    const result = await orderService.getOrdersList(request)
...
const viewInvoice = async (orderId: number) => {
  ...
  try {
    selectedInvoice.value = await orderService.getOrderInvoice(orderId)
  } catch (error: any) {
  ...
  <ClientOnly>
    <div class="compact-grid">
      <div class="mb-3 flex justify-between items-center">
        <h1 class="text-xl font-bold">مدیریت سفارشات</h1>
          ...
          <table class="min-w-full bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 text-sm">
            ...
            <tbody>
              ...
              >
                ...
                <td class="px-3 py-1.5 text-center">{{ item.orderNumber }}</td>
                <td class="px-3 py-1.5 text-center">{{ item.orderDate }}</td>
                <td class="px-3 py-1.5 text-center">{{ item.totalAmount.toLocaleString() }} تومان</td>
                <td class="px-3 py-1.5 text-center">
                  <UBadge :color="item.status === 1 ? 'warning' : item.status === 2 ? 'success' : item.status === 3 ? 'info' : item.status === 4 ? 'primary' : 'error'" variant="subtle" size="sm">
                    {{ item.statusTitle }}
                  </UBadge>
                </td>
                <td class="px-3 py-1.5 text-center">
                  <UButton size="xs" color="neutral" variant="ghost" @click="viewInvoice(item.id)">
          ...
          <div v-else-if="selectedInvoice" class="space-y-4">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-3 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
              <div>
                ...
                <div class="font-semibold">{{ selectedInvoice.orderNumber }}</div>
              </div>
              <div>
              ...
              <div>
                ...
                <UBadge :color="selectedInvoice.status === 1 ? 'warning' : selectedInvoice.status === 2 ? 'success' : selectedInvoice.status === 3 ? 'info' : selectedInvoice.status === 4 ? 'primary' : 'error'" variant="subtle" size="sm">
                ...
                <table class="min-w-full text-sm">
                  ...
                  <tbody>
                    <tr v-for="item in selectedInvoice.items" :key="item.productId" class="border-b border-gray-200 dark:border-gray-700">
                      ...
                      <td class="px-3 py-1.5 text-center">{{ item.totalPrice.toLocaleString() }} تومان</td>
                    </tr>
                  </tbody>
                </table>
              </div>
